...
class Sidebar(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        ctk.CTkFrame(self, height=2, fg_color=COLORS["bg_card"]).grid(row=15, column=0, sticky="ew", padx=20, pady=10)
        self.btn_theme = self._b(16, ar("☀️ الوضع النهاري"), COLORS["bg_input"], COLORS["bg_card_hover"], self.callbacks.get("toggle_theme"), tc=COLORS["text_secondary"])

        ctk.CTkLabel(self, text=ar("نسخة خاصة لمهندسي الشبكات"), text_color=COLORS["text_muted"], font=ctk.CTkFont(size=10)).grid(row=17, column=0, padx=20, pady=(10,15), sticky="s")

    def _b(self, row, text, color, hover, cmd=None, state="normal", tc=None):
        b = ctk.CTkButton(self, text=text, height=38, font=ctk.CTkFont(size=13),
                          fg_color=color, hover_color=hover, corner_radius=10,
                          command=cmd, state=state, text_color=tc or COLORS["text_primary"])
    ...
    def set_theme(self, mode):
        if mode == "light":
            self.btn_theme.configure(text=ar("🌙 الوضع الليلي"))
        else:
            self.btn_theme.configure(text=ar("☀️ الوضع النهاري"))
